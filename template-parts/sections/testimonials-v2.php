...
$word_limit = $args['testimonials-v2__word-limit'] ??
              get_sub_field( 'testimonials-v2__word-limit' );
...
if ( $wpdb->get_var( $wpdb->prepare( "SHOW TABLES LIKE %s", $table_name ) ) ) {
	$reviews       = $wpdb->get_results(
		$wpdb->prepare( "SELECT * FROM %i WHERE hidden = 0 AND rating = 5 ORDER BY date DESC",
			$table_name )
	);
...
if ( ! empty( $page_details['id'] ) ) {
	$google_profile_url = 'https://www.google.com/maps/search/?api=1&query=Google&query_place_id='
	                      . urlencode( $page_details['id'] );
}
        ...
        <div class="testimonials-v2__carousel" data-reveal="fade-up" data-reveal-delay="100">
            <div class="swiper js-testimonials-swiper">
                <div class="swiper-wrapper">
					...
					$short_text = $is_long ? implode( ' ', array_slice( $words, 0, $word_limit ) )
					                         . '...' : $full_text;
                    ...
                    <div class="swiper-slide">
                        <div class="testimonials-v2__card">
							...
							<?php if ( $google_profile_url ): ?>
                                <a href="<?php echo esc_url( $google_profile_url ); ?>"
                                   target="_blank" rel="noopener noreferrer"
                                   class="testimonials-v2__google-badge">
                                    <img src="https://cdn.trustindex.io/assets/platform/Google/icon.svg"
                                         alt="Google" width="30" height="30" loading="lazy">
                                </a>
							<?php else: ?>
                                <img src="https://cdn.trustindex.io/assets/platform/Google/icon.svg"
                                     alt="Google" class="testimonials-v2__google-badge" width="30"
                                     height="30" loading="lazy">
							<?php endif; ?>

							<?php if ( $google_profile_url ): ?>
                            <a href="<?php echo esc_url( $google_profile_url ); ?>" target="_blank"
                               rel="noopener noreferrer" class="testimonials-v2__avatar">
								<?php else: ?>
                                <div class="testimonials-v2__avatar">
									<?php endif; ?>
									<?php if ( ! empty( $review->user_photo ) ): ?>
                                        <img src="<?php echo esc_url( $review->user_photo ); ?>"
                                             alt="<?php echo esc_attr( $review->user ); ?>"
						...
						<?php if ( ! empty( $review->text ) ): ?>
                            <div class="testimonials-v2__text-wrap js-review-text"
							     <?php if ( $is_long ): ?>data-full="<?php echo esc_attr( $full_text ); ?>"
                                 data-short="<?php echo esc_attr( $short_text ); ?>"<?php endif; ?>>
                                <p class="testimonials-v2__text">
                                    &ldquo;<?php echo $is_long ? $short_text : $full_text; ?>&rdquo;
                                </p>
								<?php if ( $is_long ): ?>
                                    <button type="button"
                                            class="testimonials-v2__read-more js-read-more">Read
                                        more
                                    </button>
								<?php endif; ?>
                            </div>
                        ...
                        <div class="testimonials-v2__author">
                            <div class="testimonials-v2__author-info">
                                ...
                                <div class="testimonials-v2__author-info-wrapper">
									<?php if ( $review_date ): ?>
                                    ...
                                    <div class="testimonials-v2__rating">
										<?php
										$stars = ! empty( $review->rating ) ? (int) $review->rating
											: 5;
										for ( $i = 0; $i < $stars; $i ++ ):
											?>
